...
describe("GlossaryPage", () => {
  ...
  it("默认「全部」按分组渲染：通用、项目、其他桶各成一节", async () => {
    ...
    // 分组小节标题：通用 / 项目名 / 桶名，各自都出现过（chip 和分组标题都会渲染这些文字）
  ...
  it("远端 scopes 的通用 key 是「通用」而不是本地兜底的 general：不裂出两个通用分组", async () => {
    const apiClient = client({
      ...
      glossaryScopes: vi.fn().mockResolvedValue([
        { kind: "general", key: "通用", label: "通用", color: null, count: 2 },
      ]),
    });
    render(<GlossaryPage apiClient={apiClient} canWrite meetings={meetings} projects={projects} />);
    ...
    const groupHeads = screen.getAllByText("通用").filter((node) => node.tagName === "STRONG");
  ...
  it("确认建议：调用确认接口并刷新侧栏角标", async () => {
    ...
    await waitFor(() =>
      expect(apiClient.confirmGlossarySuggestion).toHaveBeenCalledWith("sug-1"),
    );

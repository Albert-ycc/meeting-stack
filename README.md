...
供 AI Agent 直接读取。**1.1 起，会议不只是被归档——会上拍板的每一条任务都会被提取、推送、逐条确认，闭环回到任务状态。**
...
录音来源
  ...
  └─ 任何方式放进监听目录的音频文件
   ...
   归档根 <YYMMDD 主题>/   音频、逐字稿、字幕、说话人分组、纪要
        │
        ▼
   workbench/  资料库、全文与语义检索、播放、逐字稿编辑、任务看板、复制路径给 Agent
        │
        ▼
   task-notify/  任务抽取 → 飞书卡片（确认/驳回按钮）→ 长连接回调 → 状态落库 → 卡片原地更新
```
...
| [`task-notify/`](task-notify/) | 任务推送与确认闭环 | 卡片构造 + 长连接回调监听（参考实现） |
